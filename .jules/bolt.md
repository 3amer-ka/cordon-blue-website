...
## 2024-05-18 - [Optimize Image Resizing Computations]
**Learning:** Redundant `resize()` calls in Pillow processing loops are costly and block the CPU. In `optimize_image.py`, processing different output formats for the exact same dimensions was causing duplicated, expensive LANCZOS resampling.
**Action:** Always maintain a local dictionary cache (keyed by final dimension tuples like `(width, height)`) of resized `Image` objects during batch processing loops to avoid recomputing identical scalings for fallback formats like JPEG.
