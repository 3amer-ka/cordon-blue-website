...
## 2026-06-25 - Preload LCP Images
**Learning:** To optimize Largest Contentful Paint (LCP) for responsive images defined via `<picture>` tags or `srcset`, adding a corresponding `<link rel="preload" as="image" imagesrcset="..." imagesizes="..." fetchpriority="high" />` to the HTML `<head>` makes them discoverable by the browser's preload scanner early.
**Action:** When adding responsive hero images above the fold, always add an accompanying `<link rel="preload">` in the `<head>` utilizing the `imagesrcset` and `imagesizes` attributes.
