...
## 2024-06-27 - Preloading LCP Responsive Images
**Learning:** To optimize Largest Contentful Paint (LCP) for responsive images defined via `<picture>` tags or `srcset`, we must add a corresponding `<link rel="preload" as="image" imagesrcset="..." imagesizes="..." fetchpriority="high" />` to the HTML `<head>`. This makes the image discoverable by the browser's preload scanner early, before the DOM is fully parsed and the `<picture>` tag is encountered.
**Action:** When adding responsive hero images below the `<head>`, always ensure a matching preload link tag is present in the `<head>` to maintain optimal LCP performance.
