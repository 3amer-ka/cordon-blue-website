...
class TestOptimizeHotelResortImage(unittest.TestCase):
    ...
    @patch('optimize_hotel_resort.Image.open')
    @patch('sys.stdout', new_callable=MagicMock)
    def test_general_exception(self, mock_stdout, mock_open):
        # Arrange
        ...
        # Act
        result = optimize_image('corrupted.jpg', './test_dir/', 'base')

        # Assert
        self.assertFalse(result)
        mock_open.assert_called_once_with('corrupted.jpg')
